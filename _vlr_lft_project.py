...
#func login
  def login():
    ...
    elif username and password == 'admin_vlr':
      admin_vlr()
...
#register
  ...
  #input_data_rank
    ...
    rank_list = ['Unranked','Iron 1','Iron 2','Iron 3','Bronze 1','Bronze 2','Bronze 3','Silver 1','Silver 2','Silver 3','Gold 1','Gold 2','Gold 3','Platinum 1','Platinum 2','Platinum 3','Diamond 1','Diamond 2','Diamond 3','Immortal 1','Immortal 2','Immortal 3','Radiant']
      ...
      if response == True:
        if len(newuser_input.get()) == 0:
          ...
          messagebox.showerror("Error","please enter your title",parent=new_window)
            ...
            messagebox.showerror("Error","invalid",parent=new_window)
#main_menu
def main():
#tkinter main
  global main_menu
...
def search_lft():
  ...
  Button(new_window,text="refresh",font=smallFont,fg="#ffffff",bg="#dc3d4b",width=8,height=1,command=lambda:SELECT()).place(x=900,y=470)
  def copy_riot_id():
...
#input_data_riot_id
    Label(new_window,text="riot id:",font=('Helvetica',12),bg="#dc3d4b").place(x=80,y=560)
    input_2_lft_Entry=Entry(new_window,text="",font=('Helvetica',12))
...
#Image need to be conver into binary before insert into database
  ...
  def insert_image():
    ...
    for image in get_image:
      ...
      c.execute("INSERT INTO report_vlr (riot_id,reason,reason_title,image) VALUES(?,?,?,?)",(report_id.get(),reason_list.get(),reason_title.get(),insert_photo)) 
                 
    conn.commit()
  Button(new_window,text="upload evidence",command=lambda:confirm_uplond(),font=MidFont,bg="#dc3d4b",fg="#ffffff").place(x=500,y=350)
...
def admin_vlr():
  ...
  Button(new_window,text="delete",width=8,height=1,command=lambda:confirm()).place(x=670,y=470)
...
#table lfp
  ...
  def delete():
      ...
      c.execute('''DELETE FROM valorant_tgt WHERE riot_id = ?''',(del_select,))
      c.execute('''DELETE FROM report_vlr WHERE riot_id = ?''',(del_select,))
      conn.commit()
